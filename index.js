...
async function run() {
    try {
        ...
        const paymentsCollection = client.db('chafferResaleServer').collection('payments');
        ...
        app.post('/create-payment-intent', async (req, res) => {
            const booking = req.body;
            const rsprice = booking.rsprice;
            const amount = rsprice * 100;
...
app.get('/', async (req, res) => {
    res.send('Chaffer resale portal server is running');
})

app.listen(port, () => console.log(`Chaffer resale portal running on ${port}`))